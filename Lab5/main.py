...
path_root = r"C:/Users/paulc/Desktop/INF582/lab5/for_moodle/"  # fill me!
...
def bidir_gru(my_seq,n_units):
    ...
    return Bidirectional(### fill the gap ### # add a default GRU layer (https://keras.io/layers/recurrent/). You need to specify only the 'units' and 'return_sequences' arguments
                         GRU(units=n_units, return_sequences=True),merge_mode='concat', weights=None)(my_seq)
...
sent_att_vec_dr = Dropout(rate=drop_rate)(sent_att_vec)
...
doc_sa = bidir_gru(sent_att_vecs_dr, n_units)
doc_att_vec, sent_att_coeffs = AttentionWithContext(return_coefficients=True)(doc_sa)
doc_att_vec_dr = Dropout(rate=drop_rate)(doc_att_vec)
...
### fill the gap (2 gaps) ###
# define a Model named 'get_word_att_coeffs' that extracts the attention coefficients of the words in a sentence

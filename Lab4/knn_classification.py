...
path_root =  r"C:/Users/paulc/Desktop/INF582/lab4/for_moodle/"# fill me!
...
for idx,doc in enumerate(cleaned_docs):
    ...
    p_embeddings = np.concatenate([my_vector_getter(token,w2v) for token in doc])
    centroid = np.mean(p_embeddings, axis=0)
...
for nn in [1,3,5,7,11,13,15,17,21,23]:
    ...
    for idx,dists in enumerate(my_distances):
        ...
        labels_nn = [labels[:n_train][elt] for elt in idxs_sorted[:nn]] # the less distant elements are the closest Apparently, it could be idxs_sorted[-nn:]
